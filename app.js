...
function simularFaturamento() {
    ...
    // ==================================================================
    // 💎 MODO PREMIUM: INSTÂNCIAS DOS GRÁFICOS DE PIZZA COM PORCENTAGEM
    // ==================================================================
    ...
    window.chartShareParcelado = new Chart(document.getElementById('graficoShareParcelado').getContext('2d'), { 
        type: 'pie', 
        data: { labels: [`Pix (${sharePix.toFixed(1)}%)`, `Débito (${shareDebito.toFixed(1)}%)`, `Crédito 1x (${share1x.toFixed(1)}%)`, `Parcelado (${shareParcelado.toFixed(1)}%)`], datasets: [{ data: [sharePix, shareDebito, share1x, shareParcelado], backgroundColor: ['#4CAF50', '#2196F3', '#FF9800', '#E91E63'] }] }, 
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { boxWidth: 12, font: { size: 11 } } } } } 
    });
    
    window.chartShareBandeiras = new Chart(document.getElementById('graficoShareBandeiras').getContext('2d'), { 
        type: 'pie', 
        data: { labels: [`Visa/Master (${percVisaMaster.toFixed(1)}%)`, `Outras (${pDemaisGeral.toFixed(1)}%)`], datasets: [{ data: [percVisaMaster, pDemaisGeral], backgroundColor: ['#0056b3', '#FFE600'] }] }, 
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { boxWidth: 12, font: { size: 11 } } } } } 
    });
}
...
function exportarRelatorio(apenasTaxas) {
    ...
    if (!apenasTaxas) {
        ...
        if (window.chartShareParcelado && document.getElementById("img_grafico_parcelado")) document.getElementById("img_grafico_parcelado").src = document.getElementById("graficoShareParcelado").toDataURL();
        if (window.chartShareBandeiras && document.getElementById("img_grafico_bandeiras")) document.getElementById("img_grafico_bandeiras").src = document.getElementById("graficoShareBandeiras").toDataURL();
